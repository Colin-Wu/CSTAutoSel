			...
			if (TotalOrderCnt.equals("1")) {
				...
				if (outIsFound.equals("1")) {
					...
					if (CommUtil.isMatchByReg(Location.toUpperCase(), vStockLocation.toUpperCase())) {
						retVal = "0";
						CommUtil.logger.info("> HP part(Warranty) successfully putaway to stock location.");
					} else {
						CommUtil.logger.info("> HP part(Warranty) failed to putaway to stock location.");
					}
