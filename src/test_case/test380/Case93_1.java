	...
	public static String run (WebDriver webdriver) throws Exception {
		...
		if (retObj != null) {
			...
			if (TotalOrderCnt.equals("1")) {
		        ...
		        CommUtil.logger.info("> add warranty part.");
    			...
    			InputGeneratePart.put("Location", vRepairLocation);
		        ...
		        CommUtil.logger.info("> check putaway to repair location successfully.");
				...
				if (outIsFound.equals("1")) {
					...
					if (CommUtil.isMatchByReg(Location.toUpperCase(), vRepairLocation.toUpperCase())) {
						retVal = "0";
						CommUtil.logger.info("> HP part(Warranty) successfully putaway to repair location.");
					} else {
						CommUtil.logger.info("> HP part(Warranty) failed to putaway to repair location.");
					}
