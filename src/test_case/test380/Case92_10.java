		...
		if (retArr.size() > 0) {
			...
			if (retObj != null) {
				...
				if (TotalOrderCnt.equals("3")) {
					...
					CommUtil.logger.info("Check Child1 is complete and attrbute is \"Internal repair of defective\"");				
					CommUtil.logger.info("> SearchOrdAdmin_CA.");
					SearchOrdAdmin_CAInputObj = new HashMap<String, String>();
					SearchOrdAdmin_CAInputObj.put("OrdNum", C1Num);
					SearchOrdAdmin_CAInputObj.put("ProjectCode", vProject);
					...
					retstr = RetObj.get("RetVal");
					OrdStatus = RetObj.get("OrdStatus");
					String OrdAttr = RetObj.get("OrdAttr");
					if(retstr.equals("0")) {
						CommUtil.logger.info("GetOrdInfoFromResult - Success");
						...
						if (OrdAttr.equals("Internal repair of defective")) {
							CommUtil.logger.info("Child1 Order changed to Internal repair of defective");
							
						} else {
							CommUtil.logger.info("Child1 OrdAttr incorrect. status:"+OrdStatus);
							return retVal;	
						}
