...
/*        	while (!TestSetting.reslist.addSN("82_13",inExpectedSendoutSN)) {
        		boxidx++;
			...
			if (retObj != null) {
				...
				if (TotalOrderCnt.equals("3")) {
					...
					ReceiveToDetailReceivingInputObj.put("PONum", C2PO);
					...
					CommUtil.logger.info(" > Detail receive Child 2 PO.");
					...
					CommUtil.logger.info(" > parent order and child1, child2 cancel button all invisible.");					
					...
					if(retVal.equals("1")) {
						CommUtil.logger.info("CancelOrderEntry1 - Success: Child2 cancel button invisible.");
					} else {
